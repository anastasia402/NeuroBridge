...
export default function QuizReviewPanel({ quizTitle, questions }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100">
        ...
        <div className="flex space-x-3">
          <Button variant="danger">Reject (Reason Required)</Button>
          <Button variant="primary" className="bg-green-600 hover:bg-green-700 text-white">Approve Quiz</Button>
        </div>
      </div>
        ...
        {questions.map((q, idx) => (
          <div key={idx} className="bg-gray-50 p-5 rounded-xl">
            <div className="flex justify-between items-start mb-4">
              <h3 className="font-bold text-gray-900"><span className="text-gray-400 mr-2">Q{idx + 1}.</span> {q.text}</h3>
              <button className="text-xs text-blue-600 font-bold bg-blue-50 px-3 py-1 rounded-md hover:bg-blue-100">Edit</button>
            </div>
